...
import com.example.todo.directory.Person;
...
public class ToDoSecurityConfig extends WebSecurityConfigurerAdapter {
			...
			public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
				try {
					...
					if (responseEntity.getStatusCode() == HttpStatus.OK) {
						...
						return User.withUsername(person.getEmail()).password(password)
								.accountLocked(!person.isEnabled()).roles(person.getRole()).build();
					}
				} catch (Exception ex) {
					ex.printStackTrace();
				}
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().requestMatchers(PathRequest.toStaticResources().atCommonLocations()).permitAll()
				...
				.httpBasic().and()
				.csrf().ignoringAntMatchers("/h2-console/**").and()		// don't apply CSRF protection to /h2-console
